...
bool ModbusRtuServer::begin(uart_port_t port, int rx_pin, int tx_pin, int rts_pin, uint32_t baud, uint8_t slave_id) {
  ...
  // The frame boundary comes out of the UART, not out of a timer here: after
  // this much silence the peripheral raises the event that ends the frame. It
  // is also the largest delay in front of any answer, so look here first if the
  // drive ever turns out to be impatient.
  const uint8_t idleSymbols = rxIdleSymbols(baud);
  // Unchecked, the driver would keep its own default and the banner below would
  // report a gap the hardware is not using.
  if (uart_set_rx_timeout(this->port_, idleSymbols) != ESP_OK) {
    ESP_LOGE(TAG, "the UART refused a frame gap of %u symbols", idleSymbols);
    return false;
  }
  // High threshold so short HCP frames are always ended by silence, not by a
  // half-full buffer.
  if (uart_set_rx_full_threshold(this->port_, RX_FULL_THRESHOLD) != ESP_OK) {
    ESP_LOGE(TAG, "the UART refused a receive threshold of %d", RX_FULL_THRESHOLD);
    return false;
  }
  ...
  ESP_LOGI(TAG,
           "RTU server on UART%d rx=%d tx=%d rts=%d %" PRIu32
           " baud 8E1, slave id %u, frame ends after %u symbols (%" PRIu32 " us of silence)",
           static_cast<int>(this->port_), rx_pin, tx_pin, rts_pin, baud, this->slave_id_, idleSymbols,
           (uint32_t) idleSymbols * (11UL * 1000000UL / baud));
  return true;
}
