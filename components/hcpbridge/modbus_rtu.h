...
/**
 * How much silence ends a frame, counted in symbol times.
 *
 * The UART itself watches the line and raises the event; nothing here polls for
 * it. Pulled out of begin() so the arithmetic can be checked on a desk: it has
 * two traps, the unit and the register width, and both are silent when wrong.
 */
constexpr uint8_t rxIdleSymbols(uint32_t baud) {
  // Eleven bits per symbol at 8E1. 1750 us is the wait the replaced library
  // used; a drive pausing mid frame would otherwise yield two useless halves.
  const uint32_t symbolUs = 11UL * 1000000UL / baud;
  if (symbolUs == 0)
    return 92;
  const uint32_t symbols = (1750UL + symbolUs - 1) / symbolUs;  // round up
  if (symbols < 4)
    return 4;
  // The driver multiplies by the symbol length and rejects what will not fit
  // its register: ten bits on the S2, S3 and C3, but seven plus a factor of
  // eight on the original ESP32, which lands lower. 92 is the value every part
  // accepts, and only because the port is clocked from APB; the REF_TICK path
  // would cap far below the floor below. Both baud rates we use are far under.
  if (symbols > 92)
    return 92;
  return (uint8_t) symbols;
}
