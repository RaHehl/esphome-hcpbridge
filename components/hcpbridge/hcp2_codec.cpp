...
void Hcp2Codec::onRequestHook(uint32_t nowMs, uint8_t fc, uint16_t c1, uint16_t c2, uint8_t command) {
  ...
  // would undo what was just announced. Asked of the pause itself rather than
  // of the reported link: an update that was announced and then abandoned used
  // to leave the sensor reading Paused for the rest of the uptime, because
  // nothing but real silence could get it out of that state again.
  if (!this->pauseRequested.load())
...
void Hcp2Codec::setCommandValuesToRead(uint32_t nowMs) {
  ...
  if (cmd == HoermannCommand::STOP) {
    ...
    this->clearIfStill(HoermannCommand::STOP);
  }
  ...
  // What came out of the slot, kept because cmd is rewritten below and the slot
  // may only be emptied of the thing this poll actually took.
  const HoermannCommand taken = cmd;
  if (cmd != HoermannCommand::NONE) {
    // A press always wins over a repeat of our own.
    this->repeatCommand = HoermannCommand::NONE;
    this->clearIfStill(taken);
    // Armed here, on the task that owns the target, and only for the run this
    // command starts. Anything left over from a press that was refused or
    // expired goes with it.
    const int wanted = this->gotoRequest.exchange(-1);
    this->state->setGotoPosition(wanted < 0 ? 0.0f : static_cast<float>(wanted) / 100.0f);
...
bool Hcp2Codec::setCommand(uint32_t nowMs, bool cond, HoermannCommand command) {
  if (cond) {
    ...
    // Paused counts as no link. The pause branch answers without consuming the
    // slot, so a press taken here would sit through the restart window and go
    // out the moment the pause ends - a door starting while the flash is being
    // written.
    if (!this->state->valid || this->pauseRequested.load()) {
      this->events.push(HcpEvent::COMMAND_REFUSED_NO_LINK, (uint16_t) command, 0, nowMs);
      return false;
    }
    // The stamp has to be in place before the slot is filled, or the bus task
    // could pair a fresh press with the previous stamp and drop it as stale.
    // Putting it back on a refused press matters just as much: leaving it would
    // restart the expiry of the press already queued, and someone tapping while
    // the drive enumerates could hold one alive until long after they walked
    // away.
    const uint32_t previousOn = this->nextCommandOn.load();
    ...
    if (!this->nextCommand.compare_exchange_strong(expected, command)) {
      this->nextCommandOn.store(previousOn);
...
bool Hcp2Codec::stopDoor(uint32_t nowMs) {
  ...
  // Stamp first: the bus task reads the slot and then the stamp, so filling the
  // slot first can pair this stop with whatever stamp was there before - zero
  // if it is the first command since boot, which reads as long expired.
  this->nextCommandOn.store(nowMs);
...
bool Hcp2Codec::setPosition(uint32_t nowMs, int setPosition) {
  ...
  if (!setCommand(nowMs, true, here < target ? HoermannCommand::OPEN : HoermannCommand::CLOSE))
    return false;
  this->gotoRequest.store(setPosition);
  return true;
}
