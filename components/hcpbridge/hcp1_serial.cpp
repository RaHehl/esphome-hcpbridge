...
// Counted rather than timed, to keep this file free of anything but the UART
// driver.
static const uint32_t HCP1_WARN_EVERY = 256;

bool Hcp1SerialServer::warn_due() { return (this->warn_seen_++ % HCP1_WARN_EVERY) == 0; }
...
void Hcp1SerialServer::poll(uint32_t timeoutMs) {
  ...
  switch (event.type) {
    ...
    default:
      ...
      // Named, not just counted: the arm catches overflow, parity and framing
      // alike, and with 255 of 256 dropped the survivor has to say which.
      if (this->warn_due())
        ESP_LOGW(TAG, "uart event %d, waiting for the next break", static_cast<int>(event.type));
