...
# Which bus this drive speaks. Fixed when the door was installed and never
# changes, so it decides what gets compiled rather than being looked at while
# running. A Series 4 and up speaks hcp2; a SupraMatic E3 speaks hcp1.
PROTOCOL_HCP2 = "hcp2"
PROTOCOL_HCP1 = "hcp1"
...
# A variant ESPHome knows but this table does not would be turned away as
# unknown, which reads like a bug to whoever owns that board. Fail here instead,
# where the fix is one line, and fail for everyone rather than only for the
# person holding the new chip.
assert set(HP_UART_COUNT) == set(VARIANTS), (
    "hcpbridge: the UART table and ESPHome's variant list disagree about "
    f"{sorted(set(HP_UART_COUNT) ^ set(VARIANTS))}"
)
            ...
            for entry in full_config.get(domain) or []:
                ...
                if entry.get("type") == entity_type:
                    raise cv.Invalid(
                        ...
                        f"Remove the {domain} with type: {entity_type}."
                    )
    ...
    cv.Schema(
        {
            ...
            # _validate_variant.
            cv.Optional(CONF_UART_NUM): cv.int_range(min=0, max=4),
...
async def to_code(config):
    ...
    # Compile-time, not a runtime branch: the drive's generation is a fact about
    # the installation, so the other protocol's code has no reason to be on the
    # device at all.
