...
class HcpBus {
 public:
  ...
  /**
   * Waits for a gap on the wire before the caller stops answering.
   *
   * Separate from announcePause because they answer different questions: one
   * asks the drive to expect silence, which only the newer bus can do, and this
   * one waits for a moment when stopping does not cut a telegram in half, which
   * both must. A restart in the middle of one is what the drive counts.
   */
...
template<typename Transport> class HcpBusOf : public HcpBus {
 public:
  ...
  void quiesce() override { this->transport_.quiesce(); }
