...
bool Hcp1Codec::submit(uint32_t nowMs, HoermannCommand cmd) {
  ...
  // Stamp before slot: the bus task reads the slot and then the stamp, so the
  // other order lets it pair a fresh press with the previous stamp and discard
  // it as stale.
...
bool Hcp1Codec::stopDoor(uint32_t nowMs) {
  // Refused rather than reported as sent: without an address nothing we queue
  // ever reaches the wire, and a cover that says it stopped a moving door when
...
size_t Hcp1Codec::answerStatus(uint32_t nowMs, uint8_t counter, uint8_t *out) {
  uint8_t doThis = 0;
  // Loaded once. Reading the slot again further down could see a different
  // press than the one this branch decided about.
  const HoermannCommand queued = this->pending_.load();
  if (queued != HoermannCommand::NONE) {
    // A press only goes out while somebody could still be standing there.
    if (nowMs - this->pendingAt_.load() > CMD_STALE_MS) {
      this->events.push(HcpEvent::COMMAND_STALE, (uint16_t) queued, 0, nowMs);
    } else {
      HoermannCommand cmd = queued;
