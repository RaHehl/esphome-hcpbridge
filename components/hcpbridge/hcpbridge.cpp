...
void HCPBridge::setup() {
  ...
  // Built here rather than held as a member: which bus this door speaks is a
  // runtime answer now, so the two on one board may differ.
...
void HCPBridge::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  UART%u  rx=%d tx=%d rts=%d", this->uart_num_, this->rx_pin_, this->tx_pin_, this->rts_pin_);
}
