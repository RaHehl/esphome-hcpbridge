...
int main() {
  ...
  while (std::getline(std::cin, line)) {
    ...
    printf(" S:%d,%d,%.3f,%.3f,%d,%d,%d,%.3f ", (int) e.state->valid, (int) e.state->state,
           e.state->targetPosition.load(), e.state->currentPosition.load(), (int) e.state->lightOn,
           (int) e.state->relayOn, (int) e.state->changed, e.state->gotoPosition);
