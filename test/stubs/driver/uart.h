...
inline esp_err_t uart_flush_input(uart_port_t) {
  g_uart.rx_pos = g_uart.rx_len;
  return 0;
}
inline int uart_read_bytes(uart_port_t, uint8_t *b, size_t n, TickType_t) {
  size_t avail = g_uart.rx_len - g_uart.rx_pos;
  size_t take = n < avail ? n : avail;
  memcpy(b, g_uart.rx + g_uart.rx_pos, take);
  g_uart.rx_pos += take;
  return (int) take;
}
inline int uart_write_bytes(uart_port_t, const void *d, size_t n) {
  memcpy(g_uart.tx + g_uart.tx_len, d, n);
  g_uart.tx_len += n;
  return (int) n;
}
inline esp_err_t uart_wait_tx_done(uart_port_t, TickType_t) { return 0; }
// The break the older bus delimits frames with. Nothing on the host can
// carry one, so it lands in the transmit buffer like any other byte.
inline int uart_write_bytes_with_break(uart_port_t, const void *d, size_t n, int) {
  memcpy(g_uart.tx + g_uart.tx_len, d, n);
  g_uart.tx_len += n;
  return (int) n;
}
// Has to come after the event struct it fills in.
inline int xQueueReceive(QueueHandle_t, void *out, TickType_t) {
  if (g_uart.ev_i >= g_uart.ev_n)
    return 0;
  uart_event_t *e = (uart_event_t *) out;
  e->type = UART_DATA;
  e->size = g_uart.ev[g_uart.ev_i++];
  return 1;  // pdTRUE
}
