...
F=[]
def add(p):
    if len(p)+2 > 256: return   # Modbus RTU has no longer frames
...
# complete frames of every function code: length always covers the declared byte count
...
# frames with trailing bytes after the declared data. The old version echoes
# the WHOLE received PDU for 0x06/0x16, trailing bytes included.
