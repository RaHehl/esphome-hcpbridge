...
def why_silence(f):
    ...
    if fc == 0x17:
        ...
        if read_cnt < 1 or read_cnt > 0x7D or write_cnt < 1 or byte_cnt != 2 * write_cnt:
...
def deliberate(frame, old_r, new_r):
    ...
    if len(new_r) >= 20 and new_r[16:20] == "0322":
        # The identity request rides along in an ordinary status answer.
