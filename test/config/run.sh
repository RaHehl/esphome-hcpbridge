...
# Belongs here rather than with the unit tests: it needs the real ESPHome, and
# this is the gate that installs it. Under the unit gate it would skip itself
# and report green.
echo "the UART table knows every chip ESPHome does:"
"$ESPHOME_PYTHON" test/unit/variant_table.py || exit 1
